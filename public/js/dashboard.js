...
async function downloadLogs(logType = 'bridge-cron') {
    try {
        const apiKey = getApiKey();
        if (!apiKey) {
            alert('API key required. Press Ctrl+K to set it.');
            return;
        }

        // Show loading message
        setActionStatus(`Downloading ${logType} logs...`, 'info');
        
        // Create download URL with proper authentication
        const url = `/maintenance/download-logs?type=${encodeURIComponent(logType)}`;
        
        // Use fetch to download with authentication headers
        ...
        let filename = `outlookbookingsync_${logType}_${new Date().toISOString().slice(0,19).replace(/[:.]/g, '-')}.log`;
        ...
        setActionStatus(`Downloaded ${filename} successfully`, 'success');
        
    } catch (error) {
        console.error('Log download failed:', error);
        setActionStatus(`Failed to download logs: ${error.message}`, 'error');
    }
}
