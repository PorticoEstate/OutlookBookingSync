...
*/5 * * * * if [ -f /scripts/enhanced_process_deletions.sh ]; then /scripts/enhanced_process_deletions.sh >> /var/log/bridge-cron.log 2>&1; else echo "$(date): Script not found: /scripts/enhanced_process_deletions.sh" >> /var/log/bridge-cron.log; fi

# 3. SYSTEM HEALTH & MONITORING
# Check bridge health every 10 minutes
*/10 * * * * curl -s -X GET "http://localhost/bridges/health" >> /var/log/bridge-cron.log 2>&1
...
0 2 * * 0 curl -s -X DELETE "http://localhost/alerts/old?days=7" >> /var/log/bridge-cron.log 2>&1

# 5. RESOURCE MAPPING MAINTENANCE
# Validate resource mappings weekly on Monday at 1 AM
0 1 * * 1 curl -s -X GET "http://localhost/mappings/resources" >> /var/log/bridge-cron.log 2>&1
